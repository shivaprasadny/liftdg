...
interface WorkoutRow { id: string; plan_id: string | null; name: string; workout_type: Workout['workoutType']; started_at: string; completed_at: string | null; duration_seconds: number | null; notes: string | null; status: Workout['status']; created_at: string; updated_at: string; paused_at?:string|null;total_paused_seconds?:number;current_exercise_id?:string|null;history_display_name?:string|null;history_notes?:string|null;corrected_local_date?:string|null;completion_quality?:Workout['completionQuality'];session_source_type?:string|null;local_workout_date?:string|null;original_snapshot_json?:string|null;current_snapshot_json?:string|null; }
interface ExerciseRow { id: string; workout_id: string; exercise_id: string; exercise_order: number; target_sets: number | null; target_reps_min: number | null; target_reps_max: number | null; target_weight: number | null; rest_seconds: number | null; notes: string | null; started_at: string | null; completed_at: string | null; session_status?:WorkoutExercise['sessionStatus']; skipped_reason?:string|null; added_during_session?:number;original_exercise_id?:string|null;replacement_status?:WorkoutExercise['replacementStatus'];replacement_audit_id?:string|null;replacement_reason?:string|null;replacement_transfer_mode?:string|null; name: string; category: Exercise['category']; primary_muscles: string | null; secondary_muscles: string | null; equipment: Exercise['equipment']; exercise_type: Exercise['exerciseType']; instructions: string | null; is_builtin: number; is_archived: number; exercise_created_at: string; exercise_updated_at: string; }
interface SetRow { id:string;workout_exercise_id:string;set_number:number;weight:number|null;reps:number|null;duration_seconds:number|null;distance:number|null;set_type:SetType;rpe:number|null;completed:number;completed_at:string|null;notes:string|null;created_at:string|null;updated_at:string|null;group_id:string|null;group_type:string|null;group_order:number|null;stage_number:number|null;assistance_weight:number|null;bodyweight_value:number|null;added_weight:number|null;round_number:number|null;target_duration_seconds:number|null;target_distance:number|null;is_amrap:number;added_during_session?:number;weight_mode?:WorkoutSet['weightMode']; }
interface HistoryRow extends WorkoutRow { plan_name:string|null;exercise_count:number;completed_set_count:number;total_repetitions:number;total_volume:number;cardio_duration_seconds:number;cardio_distance_km:number;completion_quality:'complete'|'partial'|'empty'|null;session_source_type:string|null;local_workout_date:string|null;personal_record_count:number; }
...
const mapWorkout = (row: WorkoutRow): Workout => ({ id: row.id, planId: row.plan_id, name: row.history_display_name?.trim()||row.name, workoutType: row.workout_type, startedAt: row.started_at, completedAt: row.completed_at, durationSeconds: row.duration_seconds, notes:row.history_notes??row.notes, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at,pausedAt:row.paused_at??null,totalPausedSeconds:row.total_paused_seconds??0,currentExerciseId:row.current_exercise_id??null,correctedLocalDate:row.corrected_local_date??null,completionQuality:row.completion_quality??null,sessionSourceType:row.session_source_type??null,localWorkoutDate:row.local_workout_date??null,originalSnapshotJson:row.original_snapshot_json??null,currentSnapshotJson:row.current_snapshot_json??null });
const mapSet = (row:SetRow):WorkoutSet=>({id:row.id,workoutExerciseId:row.workout_exercise_id,setNumber:row.set_number,weight:row.weight,reps:row.reps,durationSeconds:row.duration_seconds,distance:row.distance,setType:row.set_type,rpe:row.rpe,completed:row.completed===1,groupId:row.group_id,groupType:row.group_type,groupOrder:row.group_order,stageNumber:row.stage_number,assistanceWeight:row.assistance_weight,bodyweightValue:row.bodyweight_value,addedWeight:row.added_weight,roundNumber:row.round_number,targetDurationSeconds:row.target_duration_seconds,targetDistance:row.target_distance,isAmrap:row.is_amrap===1,addedDuringSession:row.added_during_session===1,weightMode:row.weight_mode??'external_weight',completedAt:row.completed_at,notes:row.notes,createdAt:row.created_at??row.completed_at??'',updatedAt:row.updated_at??row.completed_at??''});
  ...
  const now = new Date().toISOString(); await db.withExclusiveTransactionAsync(async (transaction) => {
    ...
    for (const exerciseId of exerciseIds) { const exists = await transaction.getFirstAsync('SELECT id FROM workout_exercises WHERE workout_id = ? AND exercise_id = ?', [workoutId, exerciseId]); if (exists) continue;
      const linkId = createId('workout_exercise'); await transaction.runAsync(`INSERT INTO workout_exercises
        (id, workout_id, exercise_id, exercise_order, target_sets, target_reps_min, target_reps_max, rest_seconds, started_at, original_order, added_during_session)
        VALUES (?, ?, ?, ?, 3, 8, 12, 90, ?, NULL, 1)`, [linkId, workoutId, exerciseId, order++, now]);
...
export async function pauseWorkout(db:SQLiteDatabase,id:string):Promise<void>{const now=new Date().toISOString();const result=await db.runAsync("UPDATE workouts SET paused_at=?,updated_at=? WHERE id=? AND status='active' AND paused_at IS NULL",[now,now,id]);if(result.changes!==1)throw new AppError('Workout could not be paused.');}
export async function resumeWorkout(db:SQLiteDatabase,id:string):Promise<void>{const now=new Date();const row=await db.getFirstAsync<{paused_at:string}>("SELECT paused_at FROM workouts WHERE id=? AND status='active' AND paused_at IS NOT NULL",[id]);if(!row)throw new AppError('Workout is not paused.');const seconds=Math.max(0,Math.floor((now.getTime()-new Date(row.paused_at).getTime())/1000));await db.runAsync('UPDATE workouts SET paused_at=NULL,total_paused_seconds=total_paused_seconds+?,updated_at=? WHERE id=?',[seconds,now.toISOString(),id]);}
export async function persistCurrentExercise(db:SQLiteDatabase,workoutId:string,exerciseId:string|null):Promise<void>{await db.runAsync('UPDATE workouts SET current_exercise_id=?,updated_at=? WHERE id=?',[exerciseId,new Date().toISOString(),workoutId]);}
export async function skipWorkoutExercise(db:SQLiteDatabase,id:string,reason:string|null):Promise<void>{await db.runAsync("UPDATE workout_exercises SET session_status='skipped',skipped_reason=? WHERE id=?",[reason,id]);}
export async function addWorkoutSet(db: SQLiteDatabase, workoutExerciseId: string, copy?: WorkoutSet, setType:SetType='working'): Promise<void> {
  ...
  await db.runAsync(`INSERT INTO workout_sets (id, workout_exercise_id, set_number, weight, reps, set_type, rpe, completed, notes, created_at, updated_at, original_order, added_during_session)
    VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, NULL, 1)`, [createId('workout_set'), workoutExerciseId, row?.next ?? 1, copy?.weight ?? null, copy?.reps ?? null, copy?.setType ?? setType, copy?.rpe ?? null, copy?.notes ?? null, now, now]);
}
...
export async function finishWorkout(db:SQLiteDatabase,workout:ActiveWorkout):Promise<void>{const now=new Date().toISOString();await db.withExclusiveTransactionAsync(async transaction=>{const cardio=await transaction.getFirstAsync<{count:number}>('SELECT COUNT(*) AS count FROM cardio_sessions WHERE workout_id=?',[workout.id]);const type=deriveWorkoutType(workout,cardio?.count??0);const result=await transaction.runAsync(`UPDATE workouts SET status='completed',workout_type=?,completed_at=?,duration_seconds=?,paused_at=NULL,updated_at=? WHERE id=? AND status='active'`,[type,now,elapsedSeconds(workout.startedAt,Date.now(),workout.totalPausedSeconds??0,workout.pausedAt),now,workout.id]);if(result.changes!==1)throw new AppError('Workout was already finished.');await transaction.runAsync("UPDATE scheduled_workouts SET status='completed',active_session_id=NULL,updated_at=? WHERE active_session_id=?",[now,workout.id]);});
...
const historySelect = `SELECT w.*, wp.name AS plan_name,
  ...
  COALESCE((SELECT COUNT(*) FROM personal_records pr WHERE pr.workout_id=w.id),0) AS personal_record_count
...
function mapHistory(row: HistoryRow): WorkoutHistoryItem {
  return { id: row.id, planId: row.plan_id, planName: row.plan_name, name: row.history_display_name?.trim()||row.name,
    ...
    completedSetCount:row.completed_set_count,totalRepetitions:row.total_repetitions,totalVolume:row.total_volume,cardioDurationSeconds:row.cardio_duration_seconds,cardioDistanceKm:row.cardio_distance_km,completionQuality:row.completion_quality,sourceType:row.session_source_type,localWorkoutDate:row.corrected_local_date??row.local_workout_date,personalRecordCount:row.personal_record_count };
}
...
function historyQuery(filter: WorkoutHistoryFilter, sort: WorkoutHistorySort): { sql: string; params: (string | number)[] } {
  ...
  if (term) { const like = `%${term}%`; where.push(`(COALESCE(w.history_display_name,w.name) LIKE ? COLLATE NOCASE OR COALESCE(w.history_notes,w.notes, '') LIKE ? COLLATE NOCASE
    OR COALESCE(wp.name, '') LIKE ? COLLATE NOCASE OR w.workout_type LIKE ? COLLATE NOCASE OR COALESCE(w.completion_quality,'') LIKE ? COLLATE NOCASE OR COALESCE(w.session_source_type,'') LIKE ? COLLATE NOCASE OR EXISTS (SELECT 1 FROM workout_exercises swe JOIN exercises se ON se.id = swe.exercise_id WHERE swe.workout_id = w.id AND (se.name LIKE ? COLLATE NOCASE OR se.equipment LIKE ? COLLATE NOCASE)))`); params.push(like, like, like, like, like, like, like, like); }
...
export async function deleteWorkout(db: SQLiteDatabase, id: string): Promise<void> {
  ...
  try { await db.withExclusiveTransactionAsync(async (transaction) => {const now=new Date().toISOString();await transaction.runAsync(`INSERT INTO completed_workout_deletion_audits(id,completed_workout_id,snapshot_json,deleted_at,operation_id) VALUES(?,?,?,?,?)`,[createId('history_deletion'),id,JSON.stringify(existing??{id}),now,createId('delete_operation')]); const result = await transaction.runAsync("DELETE FROM workouts WHERE id = ? AND status = 'completed'", [id]); if (result.changes !== 1) throw new AppError('Completed workout not found.'); }); } catch (error) { throw toAppError(error, 'Could not delete this workout.'); }
