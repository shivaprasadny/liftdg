import { router, useFocusEffect } from 'expo-router'; import { useCallback, useEffect, useState } from 'react'; import { ActivityIndicator, ScrollView, StyleSheet } from 'react-native';
...
export default function PlansScreen() {
  ...
  return <AppScreen header={<Header title="Plans" action={<AppButton label="New Plan" onPress={() => { reset(); router.push('/plans/create'); }} />} />}>
    <SearchBar value={search} onChangeText={setSearch} />{loading ? <ActivityIndicator style={styles.loader} size="large" color={colors.accent} /> : <ScrollView contentContainerStyle={styles.content}>
...
const styles = StyleSheet.create({ loader: { flex: 1 }, content: { paddingVertical: spacing.lg, paddingBottom: spacing.xxl, gap: spacing.md } });
