...
## Training Phase 10: Exercise Replacement

**Goal:** Safely substitute exercises without rewriting performed work. **Features:** ranked and explained alternatives, equipment/restriction context, reason and transfer selection, stable slot metadata, idempotent audit rows, and active-session partial replacement. **Depends on:** active workout snapshots and grouped exercise membership. **Done when:** active replacement passes device persistence/group tests and scheduled/program/template revision scopes and revert controls ship. **Status:** in progress — the active-session slice and schema are implemented; multi-occurrence scopes and management UI remain deferred.

## Training Phase 11: History and Review

**Goal:** Make completed training searchable, visual, and immutable by default. **Features:** paginated list, bounded calendar/timeline views, quality/PR/source badges, completed details, saved-snapshot comparison, repeat-as-new, duplicate-as-plan, correction audits, and protected deletion audits. **Depends on:** completion snapshots and confirmed PR rows. **Done when:** dedicated exercise, PR, and real active-program history views plus restore-from-deletion and physical-device accessibility checks ship. **Status:** in progress — workout-history and audit-safe correction slices are implemented; broader program lifecycle history is unavailable because there is still no `active_programs` lifecycle.
