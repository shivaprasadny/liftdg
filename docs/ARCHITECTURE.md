...
- The Training planner derives a complete visible week range with `plannerGridRange`, queries only that local-date range, renders lightweight month cells through `TrainingMonthPlanner`, and loads full workout actions only in the selected-day panel. Scheduling receives the selected canonical `YYYY-MM-DD` through Expo Router params.
