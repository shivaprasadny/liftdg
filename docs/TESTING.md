...
- [ ] The "Programs" button on the Training tab opens a list showing "Shiva's Strength & Athletic" under a featured section; opening it shows all 8 weeks with Week 4 marked Deload and Week 8 marked Assessment, each week's 4 days (Push and Core / Pull and Conditioning / Legs and Athletic Strength / Full Body) show the correct workout-type badge and exercise count, and tapping a day opens that plan's real detail screen.
- [ ] Reinstalling/reseeding does not duplicate the program, its weeks, or its days (idempotent on `program_seed_version`).
- [ ] Tapping an exercise on a plan's detail screen, or in its editor, opens that exercise's detail page.
- [ ] The "Calendar" button on the Training tab opens an Agenda list; "Schedule Workout" lets you search/pick a plan, enter a date (masked MM/DD/YYYY) and optional daypart/notes, and the new item appears under the correct day, correctly ordered against other items that day (exact time before daypart before creation order). The scheduled date never shifts by a day regardless of device timezone.
- [ ] Tapping an Agenda item opens its detail screen: date/daypart/notes are editable and save correctly ("Save Changes"), "Remove from Calendar" deletes just that item, and "View Workout" opens the underlying plan when it still exists.
- [ ] "Start Program" on a program's detail screen previews the correct end date (an 8-week program starting Monday ends the Sunday 8 weeks later), then creates one Agenda item per non-rest program day across all weeks, each showing a "Program · Week N" label; rest days never get a calendar item.
- [ ] Completing a workout with no exercise replacement shows no "Apply Changes" action; completing one that replaced an exercise (and is linked to a non-built-in plan that still has that exercise) shows it. Selecting the change and choosing "Update Workout Template" swaps that exercise in the plan (verify by reopening the plan); "No Future Changes" leaves the plan untouched. Completed workout history itself is never modified either way. A workout linked to a built-in plan, or with no linked plan at all, never shows the action.
...
Phase 10 automation covers deterministic recommendation order, curated relations, primary-muscle matches, equipment availability, hard restriction exclusions, explanation labels, and incompatible loading modes. Device checks additionally confirm partial replacement retains completed sets, group order remains usable, and replacement persists after restart.

Phase 11 automation covers same-local-day calendar aggregation, multiple-workout/quality/PR markers, timeline totals, neutral planned-versus-performed states, correction audit writes, immutable correction boundaries, deletion audit ordering, pagination, search, and sorting. Calendar layout, screen-reader summaries, and scroll restoration remain device checks.

Training planner tests cover complete visible-grid boundaries for both Monday-first and Sunday-first settings. Device checks cover small-screen cell readability, large text, month/year modal focus, multiple events, adjacent-month selection, Today navigation, theme contrast, and scheduling-date prefill.

Pure business functions should be fast unit tests with explicit boundary cases. Active-navigation tests cover boundaries, positions, direct selection, reorder/deletion recovery, group order, circuit rounds, completion, and unified cardio/timed/advanced-set behavior. History tests cover local date grouping, relational-search matching, pagination, sorting, repeat snapshots, target derivation, timing recalculation, deletion guards, and previous-performance mapping. Phase 6 tests cover pace/speed and units, cardio validation/type derivation, group conversion/order/rounds, plan-group copying, advanced-stage totals/record values, AMRAP/timed rules, bodyweight/assisted display, and double-count prevention. Phase 5 tests cover set volume and Epley one-rep-max boundaries, percent-change/comparison hiding, date-range and previous-period boundaries, daily/longest streak and weekly-average calculation, chart-grouping bucket math, exercise-progress aggregation and bodyweight detection, personal-record candidate detection, duplicate-record prevention, record recalculation, and backfill idempotency. Repository tests should focus on parameterized queries, cascades, transactions, and built-in protections. Phase 10 tests cover unit conversion (ml/L/US fl oz) and serving-amount formatting, glass-count rounding, day/week/month/quarter/year rollup math (including a leap-year and a non-leap-year February), goal-day counting, goal-met streaks, milestone-message thresholds, remaining/over-goal text, encouraging-message non-repetition, goal-history resolution and change planning (first-change backfill, today/tomorrow/all-history modes), historical period bounds and Previous/Next navigation boundaries (including the "no next period beyond today" rule and custom ranges never stepping), chart-aggregation thresholds and bucket math, hydration-calendar day-state classification, entry grouping/sorting (including completion-based day ranking), and the reset-confirmation exact-match guard. Phase 11 tests cover YouTube URL/video-ID extraction and validation across watch/shortened/Shorts/embed/mobile URL forms, the single-favorite-per-exercise clear-then-set logic (including toggle-off when re-tapping the current favorite), saved-video reorder persistence, and duplicate-save rejection. The Programs foundation (DECISIONS.md #45) is tested at the data level: `programService`'s pure formatting/derivation helpers, and a seed-data consistency suite (`programSeed.test.ts`) that guards against exactly the class of bug a schema this new invites — a program day referencing a plan ID that doesn't exist, a week count that doesn't match `durationWeeks`, or a training day missing its plan link — by asserting directly against the bundled JSON rather than a live database. The Calendar Agenda foundation (DECISIONS.md #46) tests `scheduledWorkoutService`'s pure functions: MM/DD/YYYY masking and round-trip conversion to the canonical local date, and the exact-time/daypart/created-time ordering rule used to sort and group items within a day. "Start Program" (DECISIONS.md #47) additionally tests `calculateProgramOccurrenceDate`/`calculateProgramEndDate` — the week/day-to-calendar-date math, including a case that crosses a month boundary, guarding against exactly the kind of off-by-one error that's easy to introduce in day-arithmetic code like this. The Apply Changes foundation (DECISIONS.md #48) tests `applyChangesRepository` against a mocked database: change detection is verified to return nothing for a workout with no linked plan, a built-in plan, only partial (not full) replacements, or a replacement whose original exercise no longer has a plan slot; applying is verified to update the correct `plan_exercises` row, skip (not overwrite) a change whose replacement already exists elsewhere in the plan while still applying the others, and throw rather than silently no-op when every selected change was skipped. Native focus, keyboard, timer, and accessibility behavior remains on the device checklist until a React Native component harness is added.
